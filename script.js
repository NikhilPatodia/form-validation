...
let arrowPostion = "up";
let forHide = document.querySelectorAll('.forHide')
let genderValue = "";
...
let showMe = 0;
let imgSize = document.querySelectorAll('.img-size');
let buttonP = document.querySelectorAll('.button-p')[0];
let type = "down";
let ltype = "down";
let etype = "down";
let dtype = "down";
Array.from(forHide).forEach(elem => {
...
const changeArrow = (type, category)=>{
  Array.from(imgSize).forEach((elem)=>{
...
Array.from(sort).forEach((elem)=>{
  elem.addEventListener('click', ()=>{
   ...
   if(elem.classList.contains("forName")){
    elem.value = elem.value === "accending"?"decending":"accending";
    type = type === "up"?"down":"up";
    changeArrow(type, "forName")
   }
   else if(elem.classList.contains("forLast")){
    elem.value = elem.value === "laccending"?"ldecending":"laccending";
    ltype = ltype === "up"?"down":"up";
    changeArrow(ltype, "forLast")
   }
   else if(elem.classList.contains("forEmail")){
    elem.value = elem.value === "eaccending"?"edecending":"eaccending";
    etype = etype === "up"?"down":"up";
    changeArrow(etype, "forEmail")
   }
   else if(elem.classList.contains("forDOB")){
    elem.value = elem.value === "daccending"?"ddecending":"daccending";
    dtype = dtype === "up"?"down":"up";
    changeArrow(dtype, "forDOB")
   }
  })
...
const sortingArray = (sortBy)=>{
  ...
  displayData(arr);
}else if(sortBy === "daccending" || sortBy === "ddecending"){
